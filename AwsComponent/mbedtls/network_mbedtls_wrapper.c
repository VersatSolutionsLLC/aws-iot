...
#include <stdbool.h>
#include <string.h>
#include "../../AwsComponent/common/timer_platform.h"
#include <network_interface.h>

#include "aws_iot_error.h"
#include "aws_iot_log.h"
#include "network_interface.h"
#include "../../AwsComponent/mbedtls/network_platform.h"
#include "legato.h"
...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) {
	...
	while((ret = mbedtls_ssl_handshake(&(tlsDataParams->ssl))) != 0) {
		LE_INFO("mbedtls_ssl_handshake returned -0x%x\n", -ret);
		if(ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
			LE_INFO("MBEDTLS_ERR_SSL_WANT_READ: -0x%x, MBEDTLS_ERR_SSL_WANT_WRITE: -0x%x\n", MBEDTLS_ERR_SSL_WANT_READ, MBEDTLS_ERR_SSL_WANT_WRITE);
