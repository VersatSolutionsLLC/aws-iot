...
typedef struct
{
    ...
    le_temp_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_temp_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_temp_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_temp_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_temp_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_temp disconnected\n");
}
...
)
{
    ...
    if ( strnlen(sensorName, 100) > 100 )
    {
        LE_FATAL("strnlen(sensorName, 100) > 100");
    }
    ...
    _msgPtr->id = _MSGID_le_temp_Request;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_GetSensorName;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;

    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(sensorName)) << 0);
    ...
    if (sensorName)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (sensorNameSize-1) ));
    }
...
static void _Handle_le_temp_AddThresholdEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_temp_ThresholdEventHandlerFunc_t _handlerRef_le_temp_AddThresholdEventHandler = (le_temp_ThresholdEventHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_le_temp_AddThresholdEventHandler != NULL )
    {
        _handlerRef_le_temp_AddThresholdEventHandler(sensor, threshold, contextPtr );
    }
    else
    {
...
)
{
    ...
    _clientDataPtr->handlerPtr = (le_event_HandlerFunc_t)ThresholdEventHandlerPtr;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_RemoveThresholdEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (le_temp_ThresholdEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_GetTemperature;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;

    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(temperaturePtr)) << 0);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(threshold, 100) > 100 )
    {
        LE_FATAL("strnlen(threshold, 100) > 100");
    }
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_SetThreshold;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_GetThreshold;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_temp_StartMonitoring;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_temp_AddThresholdEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_le_temp_AddThresholdEventHandler, msgRef, clientDataPtr);
            break;
