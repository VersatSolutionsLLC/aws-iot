...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("gps_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("gps_ServerHandlers", 3);
...
static void Handle_gps_GetLocation
(
...
)
{
    ...
    if ( (locationSize > 1024) &&
         (locationSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting locationSize from %zu to 1024", locationSize);
        locationSize = 1024;
    }
    if (locationSize >= UINT32_MAX)
    {
        locationSize = 0;
    }
    else
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_gps_GetLocation : Handle_gps_GetLocation(msgRef); break;
