...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("config_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("config_ServerHandlers", 3);
...
static void Handle_config_sGetValue
(
...
)
{
    ...
    if ( (keyValueSize > 1024) &&
         (keyValueSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting keyValueSize from %zu to 1024", keyValueSize);
        keyValueSize = 1024;
    }
    if (keyValueSize >= UINT32_MAX)
    {
        keyValueSize = 0;
    }
    else
    ...
    char keyValueBuffer[1025];
    char *keyValue = keyValueBuffer;
    keyValue[0] = 0;
...
static void Handle_config_iGetValue
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Define storage for output parameters
    int32_t keyValueBuffer;
    int32_t *keyValuePtr = &keyValueBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        keyValuePtr = NULL;
    }
...
static void Handle_config_fGetValue
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    double *keyValuePtr = &keyValueBuffer;
    if (!(_requiredOutputs & (1u << 0)))
    {
        keyValuePtr = NULL;
    }
...
static void Handle_config_boolGetValue
(
    le_msg_MessageRef_t _msgRef
...
static void Handle_config_sPutValue
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    if ( (paramsSize > 100) &&
         (paramsSize < UINT32_MAX) )
    {
        LE_DEBUG("Adjusting paramsSize from %zu to 100", paramsSize);
        paramsSize = 100;
    }
    if (paramsSize >= UINT32_MAX)
    {
        paramsSize = 0;
    }
    else
    ...
    // Define storage for output parameters
    char paramsBuffer[101];
    char *params = paramsBuffer;
    params[0] = 0;
    if (!(_requiredOutputs & (1u << 0)))
    {
        params = NULL;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_config_sGetValue : Handle_config_sGetValue(msgRef); break;
        case _MSGID_config_iGetValue : Handle_config_iGetValue(msgRef); break;
        case _MSGID_config_fGetValue : Handle_config_fGetValue(msgRef); break;
        case _MSGID_config_boolGetValue : Handle_config_boolGetValue(msgRef); break;
        case _MSGID_config_sPutValue : Handle_config_sPutValue(msgRef); break;
        case _MSGID_config_test : Handle_config_test(msgRef); break;
