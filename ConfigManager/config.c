...
#include "../ConfigManager/minIni.h"
...
int config_GetBoolean(const char* section, const char* key, int defaultValue,
		int* keyValue) {
	LOCK();
...
void config_sPutValue(const char* section, const char* key, const char* value) {
	...
	if (0 == n) {
		...
		char buffer[50];
		LOCK();
		int m = ini_gets(section, key, NULL, buffer, sizearray(buffer),
		CONFIG_FILE_NAME);
		UNLOCK();
		LE_DEBUG("%s = %s", key, TEMP);
		if (0 == m) {
			LE_ERROR(
					"No entry corresponding to '%s' is found under (%s) section of the configuration file(%s)",
					key, section, CONFIG_FILE_NAME);
		} else if (0 == strcmp(buffer, "NULL")) {
			LE_ERROR(
					"No value can be retrieved against '%s' under (%s) section of the configuration file(%s)",
					key, section, CONFIG_FILE_NAME);
		} else {
			LE_DEBUG(
					"Successfully add entry (%s = %s) in %s section of the configuration file %s",
					key, value, section, CONFIG_FILE_NAME);
		}
	}
}
...
COMPONENT_INIT {
	...
	if (0 == isOpen) {
		LE_FATAL("Unable to open configuration file %s",
				CONFIG_FILE_NAME);
	}
}
