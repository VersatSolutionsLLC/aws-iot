...
typedef struct
{
    ...
    le_posCtrl_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_posCtrl_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_posCtrl_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_posCtrl_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_posCtrl_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_posCtrl disconnected\n");
}
...
)
{
    ...
    le_posCtrl_ActivationRef_t _result;
    ...
    _msgPtr->id = _MSGID_le_posCtrl_Request;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_posCtrl_Release;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
